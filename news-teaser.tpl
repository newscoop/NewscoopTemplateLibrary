{{ if $iterationNo == 1 }}{{ assign var="imageRendition" value="topfront" }}{{ else }}{{ assign var="imageRendition" value="square" }}{{ /if }}

            <section class="clearfix">
            
{{* Image output - rendition must be defined in theme.xml file in theme's root folder. Here we use 'topfront' rendition for top positioned news, and 'square' for all other news *}}            
...
{{* Basic article information - title, link to full article, publish date, and section in which article is published *}}              
...
{{* Next list generates list of authors of the article. Before last name, instead of comma, word 'and' is put. Also, is author  *}}
...
{{* To show list of related articles, use this list. Here we firstly check if list starts - meaning if it has at least one element - and if so, open <ul>. Also on the end of the list code, we check if list is at end, to properly close it with </ul> *}}  
